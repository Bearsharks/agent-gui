...
Use `mem reset --yes` to drop the current collection, then run `mem index` again.
...
For command details, see the [CLI reference](cli.md#mem-reset).
...
See the [CLI reference](cli.md#mem-stats) for `stats`, and the `search` / `expand` sections for content inspection workflows.
...
1. Re-index your memory files: `mem index . --force`
...
See [Getting Started](getting-started.md#embedding-providers) for embedding provider options.
