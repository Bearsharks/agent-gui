"""Configuration system for mem.

Priority chain (lowest to highest):
  dataclass defaults → ~/.mem/config.toml → .mem/mem.toml → CLI flags
"""
...
from .metadata import MetadataConfig, metadata_config_to_dict, parse_metadata_config

GLOBAL_CONFIG_PATH = Path("~/.mem/config.toml").expanduser()
PROJECT_CONFIG_PATH = Path(".mem/mem.toml")
...
@dataclass
class SessionConfig:
    referenced_doc_include_paths: list[str] = field(default_factory=list)
...
def _default_dict() -> dict[str, Any]:
    """Return MemConfig defaults as a nested dict."""
    return config_to_dict(MemConfig())
...
def _dict_to_config(d: dict[str, Any]) -> MemConfig:
    """Convert a nested dict to a MemConfig, ignoring unknown keys."""
    kwargs: dict[str, Any] = {}
    ...
    kwargs["metadata"] = parse_metadata_config(d)
    return MemConfig(**kwargs)
...
def resolve_config(
    ...
    *,
    ...
    """Layer all config sources and return the final MemConfig.
    ...
    project_cfg = load_config_file(project_config_path or PROJECT_CONFIG_PATH)
...
def config_to_dict(cfg: MemConfig) -> dict[str, Any]:
    """Convert a MemConfig to a nested dict (for saving)."""
    payload = asdict(cfg)
    payload["metadata"] = metadata_config_to_dict(cfg.metadata)
    return payload


def get_config_value(key: str, cfg: MemConfig | None = None) -> Any:
...
def set_config_value(key: str, value: Any, *, project: bool = False) -> None:
    ...
    value:
        The value to set.
    project:
        If True, write to ``.mem/mem.toml``; otherwise to the
        global ``~/.mem/config.toml``.
    """
