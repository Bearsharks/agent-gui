...
Two backends are supported, auto-detected at runtime:
  1. ONNX Runtime (preferred) — lightweight, CPU-only, included in ``mem[onnx]``
  2. sentence-transformers CrossEncoder — PyTorch-based, included in ``mem[local]``
...
def rerank(
    ...
    *,
    ...
    Automatically selects the best available backend:
      1. ONNX Runtime (``mem[onnx]``) — preferred, lightweight
      2. sentence-transformers (``mem[local]``) — PyTorch fallback
