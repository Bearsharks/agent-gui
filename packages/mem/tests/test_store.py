...
from mem.store import MilvusStore
from mem.metadata import MetadataConfig, MetadataFieldConfig
...
def test_metadata_fields_are_stored_and_filterable(tmp_path: Path):
    db = str(tmp_path / "metadata_test.db")
    metadata_config = MetadataConfig(
        fields={
            "authority_layer": MetadataFieldConfig(type="keyword", filterable=True, default="unknown"),
            "doc_kind": MetadataFieldConfig(type="keyword", filterable=True, default="unknown"),
            "metadata_confidence": MetadataFieldConfig(type="keyword", filterable=False, default="low"),
        }
    )
    s = MilvusStore(uri=db, dimension=4, metadata_config=metadata_config)
    try:
        s.upsert(
            [
                {
                    ...
                    "source": "docs_canonical/ARCHITECTURE.md",
                    "heading": "Architecture",
                    "chunk_hash": "meta_1",
                    "heading_level": 1,
                    "start_line": 1,
                    "end_line": 2,
                    "metadata_json": '{"authority_layer":"canonical","doc_kind":"architecture","metadata_confidence":"high"}',
                    "meta_authority_layer": "canonical",
                    "meta_doc_kind": "architecture",
                },
                {
                    "embedding": [0.0, 1.0, 0.0, 0.0],
                    "content": "Generated plan",
                    "source": "docs_generated/plan.md",
                    "heading": "Plan",
                    "chunk_hash": "meta_2",
                    "heading_level": 1,
                    "start_line": 1,
                    "end_line": 2,
                    "metadata_json": '{"authority_layer":"generated","doc_kind":"plan","metadata_confidence":"high"}',
                    "meta_authority_layer": "generated",
                    "meta_doc_kind": "plan",
                },
            ]
        )
        ...
        results = s.search(
            [0.5, 0.5, 0.0, 0.0],
            query_text="architecture plan",
            top_k=10,
            filter_expr='meta_authority_layer == "canonical"',
        )

        assert len(results) == 1
        assert results[0]["chunk_hash"] == "meta_1"
