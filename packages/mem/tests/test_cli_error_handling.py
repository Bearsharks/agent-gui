...
from mem import store as store_module
from mem.cli import cli
from mem.config import ConfigEnvVarError, MemConfig
...
def test_stats_shows_friendly_milvus_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli_module, "resolve_config", lambda _overrides=None: MemConfig())
...
def test_missing_env_var_in_real_resolve(monkeypatch) -> None:
    ...
    from mem import config as config_module
