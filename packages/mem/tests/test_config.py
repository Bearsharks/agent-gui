...
def test_default_config():
    """MemConfig() should produce sensible defaults."""
    cfg = MemConfig()
    assert cfg.milvus.uri == "~/.mem/index/milvus.db"
    assert cfg.milvus.collection == "mem_chunks"
    assert cfg.embedding.provider == "openai"
    assert cfg.chunking.max_chunk_size == 1500
    assert cfg.chunking.overlap_lines == 2
    assert cfg.metadata.fields == {}
...
def test_metadata_config_from_project_toml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    cfg_file = tmp_path / ".mem" / "mem.toml"
    save_config(
        {
            "metadata": {
                "extractor": {"path": "scripts/docs_metadata.py", "function": "metadata_for_path"},
                "fields": {
                    "authority_layer": {"type": "keyword", "filterable": True, "default": "unknown"},
                    "metadata_confidence": {"type": "keyword", "filterable": False, "default": "low"},
                },
            }
        },
        cfg_file,
    )
    monkeypatch.setattr("mem.config.GLOBAL_CONFIG_PATH", tmp_path / "nope-global.toml")
    monkeypatch.setattr("mem.config.PROJECT_CONFIG_PATH", cfg_file)

    cfg = resolve_config()

    assert cfg.metadata.extractor.path == "scripts/docs_metadata.py"
    assert cfg.metadata.extractor.function == "metadata_for_path"
    assert cfg.metadata.fields["authority_layer"].filterable is True
    assert cfg.metadata.fields["authority_layer"].default == "unknown"
    assert cfg.metadata.fields["metadata_confidence"].filterable is False


def test_session_config_from_project_toml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    cfg_file = tmp_path / ".mem" / "mem.toml"
    save_config(
        {
            "session": {
                "referenced_doc_include_paths": ["docs", "docs_canonical"],
            }
        },
        cfg_file,
    )
    monkeypatch.setattr("mem.config.GLOBAL_CONFIG_PATH", tmp_path / "nope-global.toml")
    monkeypatch.setattr("mem.config.PROJECT_CONFIG_PATH", tmp_path / "nope-project.toml")

    cfg = resolve_config(project_config_path=cfg_file)

    assert cfg.session.referenced_doc_include_paths == ["docs", "docs_canonical"]
...
def test_resolve_priority(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    # Patch the paths
    monkeypatch.setattr("mem.config.GLOBAL_CONFIG_PATH", global_cfg)
    monkeypatch.setattr("mem.config.PROJECT_CONFIG_PATH", tmp_path / "nope.toml")
...
def test_set_get_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("mem.config.GLOBAL_CONFIG_PATH", cfg_path)
    monkeypatch.setattr("mem.config.PROJECT_CONFIG_PATH", tmp_path / "nope.toml")
...
def test_set_config_value_writes_to_project_config_when_requested(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    global_cfg = tmp_path / "global.toml"
    project_cfg = tmp_path / ".mem/mem.toml"
    monkeypatch.setattr("mem.config.GLOBAL_CONFIG_PATH", global_cfg)
    monkeypatch.setattr("mem.config.PROJECT_CONFIG_PATH", project_cfg)
...
def test_save_config_expands_user_in_string_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    path = "~/.mem/test-config.toml"
    data = {"embedding": {"provider": "google"}}
    save_config(data, path)

    saved_path = fake_home / ".mem" / "test-config.toml"
...
def test_load_config_file_expands_user_in_string_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    loaded = load_config_file("~/.mem/test-config.toml")
    assert loaded == data
...
def test_resolve_env_refs_in_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("mem.config.GLOBAL_CONFIG_PATH", cfg_file)
    monkeypatch.setattr("mem.config.PROJECT_CONFIG_PATH", tmp_path / "nope.toml")
...
def test_dict_to_config_ignores_unknown_fields_and_non_dict_sections() -> None:
    ...
    cfg = _dict_to_config(
        {
            ...
            "milvus": "not-a-dict",
            "compact": {"llm_provider": "anthropic"},
            "watch": {"debounce_ms": 250},
            "llm": {"provider": "openai"},
            "prompts": {"compact": "prompts/compact.txt"},
    ...
    assert cfg.milvus.uri == "~/.mem/index/milvus.db"
    assert not hasattr(cfg, "compact")
    assert not hasattr(cfg, "watch")
...
def test_dict_to_config_accepts_empty_section_dicts() -> None:
    ...
    cfg = _dict_to_config({"embedding": {}, "milvus": {}, "chunking": {}, "reranker": {}})
