...
const withIframe = applyGraphPlanMutations(basePlan, [
  {
    ...
    iframe: { id: "iframe-test", description: "Test iframe", url: "http://localhost:8787/prototypes/test.html", entryPath: "docs/prototypes/test.html" },
  },
]);
...
assert.equal(designNode?.iframes?.find((iframe) => iframe.id === "iframe-test")?.entryPath, "docs/prototypes/test.html");
...
const updated = applyGraphPlanMutations(withIframe, [
  {
    ...
    fields: { description: "Updated test iframe", entryPath: "docs/prototypes/test.html" },
  },
]);
...
assert.equal(
  updated.graphs[0].nodes.find((node) => node.id === "n-design")?.iframes?.find((iframe) => iframe.id === "iframe-test")?.entryPath,
  "docs/prototypes/test.html",
);
