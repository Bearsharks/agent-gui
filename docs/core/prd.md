...
현재 제품 계약은 skill-scaffolded `.agent-gui` sandbox와 대상 프로젝트 설정/entry의 분리다.

```txt
target-project/.agents/skills/plan-gui-mcp
target-project/.agent-gui/preview.config.ts
target-project/.agent-gui/previews/**/*.preview.tsx
target-project/.agent-gui/preview-runtime
```

Skill scaffold는 local web server, shell, preset component, 내부 Vite app, virtual registry 생성을 제공한다. 실제 preview/prototype TSX entry, mock data, 상태 fixture, 디자인시스템 연결은 대상 프로젝트가 소유한다.
